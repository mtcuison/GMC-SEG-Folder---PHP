...
if(isset($_GET['typ'])){
    $doctypex = base64_decode(htmlspecialchars($_GET['typ']));
} else showMessage("Invalid parameter detected. (1)");

if(isset($_GET['nox'])){
    $docnoxxx = base64_decode(htmlspecialchars($_GET['nox']));
} else showMessage("Invalid parameter detected. (2)");

if(isset($_GET['mob'])){
    $mobileno = base64_decode(htmlspecialchars($_GET['mob']));
...
//get validator
$validator = (new FBPromoValidatorFactory())->make($app, $branchcd, $doctypex, $docnoxxx, $mobileno);
...
<body>
	...
	<script>     
    	window.fbAsyncInit = function() {
            ...
            FB.init({
              ...
              channelUrl : 'https://restgk.guanzongroup.com.ph/xtest/channel.html',
              status: true,
              cookie: true, 
              xfbml: true
            });
    ...
    <a href="javascript:void(0);" onclick="fbLogin();" id="fbLink" class="btn btn-primary btn-block"><i class="fab fa-facebook-square"></i> Login with Facebook!</a>
