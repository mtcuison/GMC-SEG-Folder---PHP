<?php
//integsys/dcp/request_mobile_update.php
...
$params = array("sTransNox", "sClientID", "cReqstCDe", "sMobileNo", "cPrimaryx", "sRemarksx", "sSourceCD", "sSourceNo");
...
$sql = "SELECT sTransNox FROM Mobile_Update_Request WHERE sTransNox = " . CommonUtil::toSQL($sTransNox);
...
$nextcd = CommonUtil::GetNextCode("Mobile_Update_Request", "sTransNox", $year, $app->getConnection(), "MX01");

$sql = "INSERT INTO Mobile_Update_Request SET" .
...
if($app->execute($sql) <= 0){ //do not execute to replication
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Unable to save Mobile_Update_Request:" . $app->getErrorMessage();
    echo json_encode($json);
    return;
}
