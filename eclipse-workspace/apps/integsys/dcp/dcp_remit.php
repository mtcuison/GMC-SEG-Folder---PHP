...
$params = array("sTransNox", "nEntryNox", "dTransact", "cRemitTyp", "sCompnyNm", "sBankAcct", "sReferNox", "cPaymType", "nAmountxx");
...
for($ctr=0; $ctr<$rows_found; $ctr++){
    ...
    if(empty($parjson[$params[$ctr]])){
        if ($params[$ctr] != "cRemitTyp" && $params[$ctr] != "cPaymType" && $params[$ctr] != "sReferNox"){
            $json["result"] = "error";
            $json["error"]["code"] = AppErrorCode::INVALID_PARAMETER;
            $json["error"]["message"] = "Empty PARAMETER detected. " . $params[$ctr];
            echo json_encode($json);
            return;
        }
    }
...
$entrynox = $parjson["nEntryNox"];
$transact = $parjson["dTransact"];
$remittyp = $parjson["cRemitTyp"];
$compnynm = $parjson["sCompnyNm"];
$bankacct = $parjson["sBankAcct"];
$refernox = $parjson["sReferNox"];
$paymtype = $parjson["cPaymType"];
$amountxx = $parjson["nAmountxx"];

//validate the refernox
if ($remittyp != "0" && $refernox == ""){
    $json["result"] = "error";
    $json["error"]["code"] = AppErrorCode::INVALID_PARAMETER;
    $json["error"]["message"] = "Empty PARAMETER detected. sReferNox";
    echo json_encode($json);
    return;
...
$sql = "INSERT INTO LR_DCP_Collection_Remittance SET" .
            "  sTransNox = " . CommonUtil::toSQL($transnox) .
            ", nEntryNox = " . CommonUtil::toSQL($entrynox) .
            ", dTransact = " . CommonUtil::toSQL($transact) .
            ", cRemitTyp = " . CommonUtil::toSQL($remittyp) .
            ", sCompnyNm = " . CommonUtil::toSQL($compnynm) .
            ", sBankAcct = " . CommonUtil::toSQL($bankacct) .
            ", sReferNox = " . CommonUtil::toSQL($refernox) .
            ", cPaymType = " . CommonUtil::toSQL($paymtype) .
            ", nAmountxx = " . CommonUtil::toSQL($amountxx) . 
...
if($app->execute($sql) <= 0){ //do not execute to replication
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Unable to save LR_DCP_Collection_Remittance: " . $app->getErrorMessage();
    echo json_encode($json);
    return;
}
