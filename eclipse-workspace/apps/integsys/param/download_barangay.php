<?php    
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Barangay Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Barangay record detected! ";
        echo json_encode($json);
        return;
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        $detail[$ctr]["sBrgyIDxx"] = mb_convert_encoding($rows[$ctr]["sBrgyIDxx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sBrgyName"] = mb_convert_encoding($rows[$ctr]["sBrgyName"], 'UTF-8', 'ISO-8859-1');
