<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sBankName LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sBankIDxx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sBankName = " . CommonUtil::toSQL($search));
        }
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        $detail[$ctr]["sBankIDxx"] = $rows[$ctr]["sBankIDxx"];
        $detail[$ctr]["sBankName"] = mb_convert_encoding($rows[$ctr]["sBankName"], 'UTF-8', 'ISO-8859-1');
