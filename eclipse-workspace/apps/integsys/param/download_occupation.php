<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sOccptnNm LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sOccptnID = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sOccptnNm = " . CommonUtil::toSQL($search));
        }
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        $detail[$ctr]["sOccptnID"] = mb_convert_encoding($rows[$ctr]["sOccptnID"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sOccptnNm"] = mb_convert_encoding($rows[$ctr]["sOccptnNm"], 'UTF-8', 'ISO-8859-1');
