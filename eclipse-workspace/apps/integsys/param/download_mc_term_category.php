<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sMCCatIDx LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading MC Term Category Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no MC Term Category record detected! ";
        echo json_encode($json);
        return;
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        ...
        $detail[$ctr]["nAcctTerm"] = mb_convert_encoding($rows[$ctr]["nAcctTerm"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["nAcctThru"] = mb_convert_encoding($rows[$ctr]["nAcctThru"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["nFactorRt"] = $aes->encrypt(mb_convert_encoding($rows[$ctr]["nFactorRt"], 'UTF-8', 'ISO-8859-1'));
