<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sTownName LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sTownIDxx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sTownName = " . CommonUtil::toSQL($search));
        }
    }
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Town/City Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Town/City record detected! ";
        echo json_encode($json);
        return;
    }
