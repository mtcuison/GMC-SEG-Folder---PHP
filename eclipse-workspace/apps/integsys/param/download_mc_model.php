<?php    
    ...
    $sql = "SELECT" .
                ...
                ", a.dTimeStmp" .
            " FROM MC_Model a" .
                " LEFT JOIN MC_Model_Price b" .
                    " ON a.sModelIDx = b.sModelIDx" .
            " WHERE a.cRecdStat = '1'" .
            " HAVING NOT ISNULL(b.sMCCatIDx)";
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "a.sModelIDx LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "a.sModelIDx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sModelNme = " . CommonUtil::toSQL($search));
        }
    }
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Model Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Model record detected! ";
        echo json_encode($json);
        return;
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        $detail[$ctr]["sModelIDx"] = mb_convert_encoding($rows[$ctr]["sModelIDx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sModelCde"] = mb_convert_encoding($rows[$ctr]["sModelCde"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sModelNme"] = mb_convert_encoding($rows[$ctr]["sModelNme"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sBrandIDx"] = mb_convert_encoding($rows[$ctr]["sBrandIDx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["cMotorTyp"] = mb_convert_encoding($rows[$ctr]["cMotorTyp"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["cRegisTyp"] = mb_convert_encoding($rows[$ctr]["cRegisTyp"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["cEndOfLfe"] = mb_convert_encoding($rows[$ctr]["cEndOfLfe"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["cEngineTp"] = mb_convert_encoding($rows[$ctr]["cEngineTp"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["cHotItemx"] = mb_convert_encoding($rows[$ctr]["cHotItemx"], 'UTF-8', 'ISO-8859-1');
