<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sModelIDx LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sModelIDx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sModelIDx = " . CommonUtil::toSQL($search));
        }
    }
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Model Price Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Model Price record detected! ";
        echo json_encode($json);
        return;
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        ...
        $detail[$ctr]["nSelPrice"] = $aes->encrypt(mb_convert_encoding($rows[$ctr]["nSelPrice"], 'UTF-8', 'ISO-8859-1'));
        $detail[$ctr]["nLastPrce"] = $aes->encrypt(mb_convert_encoding($rows[$ctr]["nLastPrce"], 'UTF-8', 'ISO-8859-1'));
        $detail[$ctr]["nDealrPrc"] = $aes->encrypt(mb_convert_encoding($rows[$ctr]["nDealrPrc"], 'UTF-8', 'ISO-8859-1'));
        $detail[$ctr]["nMinDownx"] = $aes->encrypt(mb_convert_encoding($rows[$ctr]["nMinDownx"], 'UTF-8', 'ISO-8859-1'));
        $detail[$ctr]["sMCCatIDx"] = mb_convert_encoding($rows[$ctr]["sMCCatIDx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["dPricexxx"] = mb_convert_encoding($rows[$ctr]["dPricexxx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["dInsPrice"] = mb_convert_encoding($rows[$ctr]["dInsPrice"], 'UTF-8', 'ISO-8859-1');
