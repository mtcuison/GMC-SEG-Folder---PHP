<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sProvName LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sProvIDxx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sProvName = " . CommonUtil::toSQL($search));
        }
    }
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Province Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Province record detected! ";
        echo json_encode($json);
        return;
    }
