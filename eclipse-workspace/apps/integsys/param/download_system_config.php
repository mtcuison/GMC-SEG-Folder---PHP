<?php    
    ...
    $sql = "SELECT" .
                "  a.sConfigCd" .
                ", a.sConfigDs" .
                ", IFNULL(b.sConfigVl, a.sConfigVl) sConfigVl" .
                ", a.dTimeStmp" . 
            " FROM xxxSysConfig a" .
                " LEFT JOIN xxxSysConfigHistory b" .
                    " ON a.sConfigCd = b.sConfigCd" .
                        " AND " . CommonUtil::toSQL($date->format(CommonUtil::format_date)) . " BETWEEN b.dDateFrom AND b.dDateThru" .
            " WHERE a.dDteAdded <= " . CommonUtil::toSQL($date->format(CommonUtil::format_date)) .
                " AND a.sConfigCd IN ('dcp.coordinates.capturing.interval', 'lr.field.collection.rebate')";
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "a.sConfigDs LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "a.sConfigCd = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "a.sConfigDs = " . CommonUtil::toSQL($search));
        }
    }
    ...
    if($rows === null){
        ...
        $json["error"]["message"] = "Failed loading Country Info.! " . $app->getErrorMessage();
        echo json_encode($json);
        return;
    }
    ...
    if(empty($rows)){
        ...
        $json["error"]["message"] = "There are no Country record detected! ";
        echo json_encode($json);
        return;
    }
