<?php    
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sRelgnNme LIKE " . CommonUtil::toSQL($search . "%"));
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sRelgnIDx = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sRelgnNme = " . CommonUtil::toSQL($search));
        }
    }
    ...
    for($ctr=0;$ctr<$rows_found;$ctr++){
        $detail[$ctr]["sRelgnIDx"] = mb_convert_encoding($rows[$ctr]["sRelgnIDx"], 'UTF-8', 'ISO-8859-1');
        $detail[$ctr]["sRelgnNme"] = mb_convert_encoding($rows[$ctr]["sRelgnNme"], 'UTF-8', 'ISO-8859-1');
