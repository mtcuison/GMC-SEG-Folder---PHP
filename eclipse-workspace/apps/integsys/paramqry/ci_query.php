<?php
    ...
    $bbycode = false;
    $search = "";
    if(isset($parjson['employid'])){
        $bbycode = true;
        $search = htmlspecialchars($parjson['employid']);
    } else {
        if(isset($parjson['employnm'])){
            $search = htmlspecialchars($parjson['employnm']);
        } else {
            $json["result"] = "error";
            $json["error"]["message"] = "Unset EMPLOYEE NAME detected";
            echo json_encode($json);
            return false;
        }
    }
    ...
    $sql = "SELECT" .
                "  a.sEmployID employid" .
                ", b.sCompnyNm employnm" .
