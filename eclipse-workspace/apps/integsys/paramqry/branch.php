<?php
    ...
    if(isset($parjson['branchcd'])){
        $bbycode = true;
        $search = htmlspecialchars($parjson['branchcd']);
    } else {
        if(isset($parjson['branchnm'])){
            $search = htmlspecialchars($parjson['branchnm']);
        } else {
            $json["result"] = "error";
            $json["error"]["message"] = "Unset BRANCH NAME detected";
            echo json_encode($json);
            return false;
        }
    }
    ...
    if ($bsearch == true){
        $sql = CommonUtil::addCondition($sql, "sBranchNm LIKE " . CommonUtil::toSQL($search . "%"));
        $sql = CommonUtil::addCondition($sql, "cRecdStat = '1'");
    } else {
        if ($bbycode == true){
            $sql = CommonUtil::addCondition($sql, "sBranchCd = " . CommonUtil::toSQL($search));
        }else {
            $sql = CommonUtil::addCondition($sql, "sBranchNm = " . CommonUtil::toSQL($search));
        }
    }
