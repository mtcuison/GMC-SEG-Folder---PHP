<?php
    ...
    if ($logno == ""){       
        ...
        if ($email == "xurpas7@gmail.com"){
            if (!$app->Login($email, "123456", $prodctid, $pcname, "", $clientid)){
                $json["result"] = "error";
                $json["error"]["message"] = $app->getErrorMessage();
                echo json_encode($json);
                return;
            ...
            $json["result"] = "error";
            $json["error"]["message"] = "Given EMAIL is different from SYSADMIN Account.";
            echo json_encode($json);
            return;
        }
    ...
    } else if(strtolower($approved) == "na"){
        $reasonxx = "***" . $reasonxx;
