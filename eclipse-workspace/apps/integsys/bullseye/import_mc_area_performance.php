<?php
/* https://restgk.guanzongroup.com.ph/integsys/bullseye/import_mc_area_performance.php
 * 
...
$sql = "SELECT" .
            ...
            ", a.nJOActual" .
            ", a.nLRActual" .
        " FROM MC_Area_Performance a" .
            ", Branch_Area b" .
        " WHERE a.sAreaCode = b.sAreaCode" .
            " AND a.sPeriodxx = " . CommonUtil::toSQL($period);

if ($areacd != ""){
    $sql = CommonUtil::addcondition($sql,  CommonUtil::toSQL($areacd));
}
...
if($rows === null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed loading MC area performance record.";
    echo json_encode($json);
    return;
}
if(empty($rows)){
    ...
    $json["error"]["message"] = "MC area performance for the period is not available!";
    echo json_encode($json);
    return;
}
...
for($ctr=0; $ctr < $rows_found; $ctr++){    
    $detail[$ctr]["sAreaCode"] = $rows[$ctr]["sAreaCode"];
    $detail[$ctr]["sAreaDesc"] = $rows[$ctr]["sAreaDesc"];
