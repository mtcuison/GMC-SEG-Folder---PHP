...
if(isset($parjson['maskname'])){
    $maskname = strtoupper($parjson['maskname']);
}
...
switch($maskname){
    case "":
    case "GUANZON":
        sendAsOthers("GUANZON", $message, $mobileno, $user, $pass);
        return;
    case "CARTRADE":
    case "LOSPEDRITOS":
        //$user = MASK_USER2;
        ...
        sendAsOthers($maskname, $message, $mobileno, $user, $pass);
        return;
    default:
        $json["result"] = "error";
        $json["error"]["code"] = "101";
        $json["error"]["message"] = "MASKNAME is not VALID.";
        echo json_encode($json);
        return;
}

function sendAsGuanzon($message, $mobileno){
...
function sendAsOthers($maskname, $message, $mobileno, $user, $pass){
    $header = array();
    $header[] = "Accept: application/json";
    $header[] = "Content-Type: application/json";
    
    $xparam = array();
    $xparam["username"] = $user;
    $xparam["password"] = $pass;
    $xparam["text"] = $message;
