 ...
 * use this API in requesting list of offline transaction(GuanzonApp).
 * Note:
 */
...
if($rows === null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed loading offline transaction! " . $app->getErrorMessage() ;
    echo json_encode($json);
    return;
}
if(empty($rows)){
    ...
    $json["error"]["message"] = "There are no offline transaction detected!";
    echo json_encode($json);
    return;
}
