<?php
/*
 * import_trans_preorder.php
 * kalyptus - 2019.06.22 10:24am
 * use this API in requesting list of pre-ordered transaction(GuanzonApp).
 * Note:
 */
...
$sql = "SELECT" .
              "  a.sGCardNox" . 
              ", a.dPickupxx dTransact" . 
              ", a.sTransNox sReferNox" . 
              ", b.sPromDesc sTranType" . 
              ", a.sReferNox sSourceNo" . 
              ", a.nPointsxx" .
...
if($rows === null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed loading pre-order transaction! " . $app->getErrorMessage();
    echo json_encode($json);
    return;
}
if(empty($rows)){
    ...
    $json["error"]["message"] = "There are no pre-order transaction detected!";
    echo json_encode($json);
    return;
}
