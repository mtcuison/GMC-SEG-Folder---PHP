...
if($psSerialID != ''){
    ...
    if($app->execute($sql, "MC_Serial_Service") <= 0){
        $json["result"] = "error";
        $json["error"]["message"] = "Unable to update MC Serial Service record..." . $sql;
        $app->rollbackTrans();
        echo json_encode($json);
        return;
    }
...
if($app->execute($sql, "G_Card_Master") <= 0){
    $json["result"] = "error";
    $json["error"]["message"] = "Unable to update MC Serial Service record...";
    $app->rollbackTrans();
    echo json_encode($json);
    return;
}
...
function createGCardNo($branch, $type){
    ...
    elseif(empty($rows)){
        ...
        $newgcard = $branch . $year . sprintf('%086', 1) . $type . rand(0, 9);
    }
    else{
