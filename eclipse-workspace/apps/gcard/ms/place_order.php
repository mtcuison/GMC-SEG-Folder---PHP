...
if($rows == null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed on loading the GCard information! " . $app->getErrorMessage() . $sql;
    echo json_encode($json);
    return null;
}
elseif(empty($rows)){
...
if($rows === null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed loading unclaim orders! " . $app->getErrorMessage();
    echo json_encode($json);
    return;
}
if(!empty($rows)){
...
$transno = CommonUtil::GetNextCode("G_Card_Order_Master", "sTransNox", $year, $app->getConnection(), "MX01");
...
if($app->execute($sql, "G_Card_Order_Master", $branch, "") <= 0){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed saving order... " . $app->getErrorMessage();
    $app->rollbackTrans();
    echo json_encode($json);
    return;
}
...
for($ctr=0;$ctr<$detail_sent; $ctr++){
    ...
    if($rows === null){
        $json["result"] = "error";
        $json["error"]["code"] = $app->getErrorCode();
        $json["error"]["message"] = "Failed loading redeemable items! " . $app->getErrorMessage();
        $app->rollbackTrans();
        echo json_encode($json);
        return;
    }
    if(empty($rows)){
        $json["result"] = "error";
        $json["error"]["code"] = AppErrorCode::RECORD_NOT_FOUND;
        $json["error"]["message"] = "Item is no longer available for the period!" . $sql . $data;
        $app->rollbackTrans();
        echo json_encode($json);
        return;
    }
...
if($availp - ($orderx + $orderc) < 0){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Total available points is less than the placed orders! " . $app->getErrorMessage();
    $app->rollbackTrans();
    echo json_encode($json);
    return;
}
