...
$rows_found = sizeof($parjson["detail"]);
for($ctr=0;$ctr<$rows_found; $ctr++){

    $gcardnox = $parjson["detail"][$ctr]["sGCardNox"];    
    $pointsxx = $parjson["detail"][$ctr]["nPointsxx"] * -1;

    //TODO: Insert detail here...
    $date = new DateTime('now');
    $year = $date->format("Y");
    $batchno = CommonUtil::GetNextCode("G_Card_Detail_Offline", "sTransNox", $year, $app->getConnection(), "MX01");
    
    $stamp = $date->format(CommonUtil::format_timestamp);
    $sql = "INSERT INTO G_Card_Detail_Offline" .
        " SET sTransNox = '$batchno'" .
        ", sGCardNox = '$gcardnox'" .
        ", sCompnyID = '$compnyid'" .
        ", dTransact = '$trandate'" .
        ", sSourceNo = '$transnox'" .
        ", sSourceCd = 'M00119000002'" .
        ", nTranAmtx = $pointsxx" .
        ", nPointsxx = $pointsxx" .
        ", sOTPasswd = ''" .
        ", cTranStat = '1'" .
        ", sModified = '$userid'" .
        ", dModified = '$stamp'";
    if($app->execute($sql, "G_Card_Detail_Offline", $compnyid) <= 0){
        $json["result"] = "error";
        $json["error"]["code"] = $app->getErrorCode();
        $json["error"]["message"] = "Unable to insert point transfer to master. " . $app->getErrorMessage();
        $app->rollbackTrans();
