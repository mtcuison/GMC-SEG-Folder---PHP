<?php
/*
 * import_trans_online.php
 * kalyptus - 2019.06.22 10:24am
 * use this API in requesting list of online transaction(GuanzonApp).
 * Note:
 */
...
$sql = "SELECT a.sGCardNox, a.dTransact, a.sTransNox sReferNox, b.sDescript sTranType, a.sSourceNo, a.nPointsxx" .
        " FROM G_Card_Detail a" .
            " LEFT JOIN G_Card_Points_Basis b ON a.sSourceCD = b.sSourceCd"  .
...
/*if($rows === null){
    $json["result"] = "error";
    $json["error"]["code"] = $app->getErrorCode();
    $json["error"]["message"] = "Failed loading online transaction! " . $app->getErrorMessage();
    echo json_encode($json);
    return;
}*/
if(empty($rows)){
    ...
    $json["error"]["message"] = "There are no online transaction detected!";
    echo json_encode($json);
    return;
}
